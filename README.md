...
| **Concordance (Bland–Altman)** | biais = écart moyen `transmissions − banc` sur les paliers ; limites de concordance = `biais ± 1,96 σ` | comparaison sur les **paliers**, pas sur les échantillons : sur ces derniers, un retard de quelques dizaines de ms produirait pendant les transitoires des écarts qui ne doivent rien à l'accord des méthodes. Abscisse = moyenne des deux méthodes, comme dans la méthode d'origine. La **tendance** de l'écart avec le niveau est testée : si elle est significative, des limites constantes sont trompeuses et la figure le dit |
